...
def copy_inverse_program_state_dict(static_dict, tensor_dict):
    new_state_dict = dict()
    for n, p in static_dict.items():
        new_state_dict[n] = tensor_dict[p.name]
    return new_state_dict
...
def do_train(args):
    ...
    program_state_dict = paddle.static.load_program_state(
        "/ssd1/zhonghui03/GPT2ModelCheckpoints/model_50000")
    for k, v in program_state_dict.items():
        print(k, type(v), v)
    new_dict = copy_inverse_program_state_dict(state_dict, program_state_dict)
    for k, v in new_dict.items():
        print(k, type(v), v.shape)
    paddle.save(new_dict, "./gpt.pdparams")
    exit(0)
